...
from api.views import AddListConsumers, ListCreateConsumerAddress, \
    ...
    ListConsumptionReading

urlpatterns = [
    url(r'^consumers/$', AddListConsumers.as_view(),
        name='get_create_consumer'),
    ...
    url(r'^read_consumption_readings/$',
        ListCreateConsumptionReadings.as_view(),
        name='read_consumption_readings'),
    url(r'^list_consumption_reading/(?P<meter_no>[0-9]{10})/$',
        ListConsumptionReading.as_view(),
        name='list_consumption_reading'),
    url(
        r'^avg_munic_consumption/(?P<munic_name>[a-zA-Z]+[a-zA-Z0-9]?)/(?P<date>[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z)/$',
